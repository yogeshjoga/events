...
//            "ORDER BY re.gold_in_gm DESC limit 5")
    ...
    @Query("SELECT new org.api.events.dto.TopFiveRelatives(" +
            ...
            "re.gold_in_gm) " +
            "FROM relative r " +
            "JOIN r.receivings re " +
            "WHERE r.user.id = :userId " +
            "ORDER BY re.gold_in_gm DESC")

    List<TopFiveRelatives> findTopFiveRelativesByUserIdGold(@Param("userId") UUID userId);


    @Query("SELECT SUM(r.gold_in_gm) FROM receiving r WHERE r.user.id = :userId")
    Double findTotalGold_in_gmAndUserid(@Param("userId") UUID userId);


    @Query("SELECT SUM(r.silver_in_gm) FROM presentations r WHERE r.user.id = :userId")
